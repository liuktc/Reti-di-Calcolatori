        ...
        try{ 
            while (true){// ciclo infinito del server
                ...
                try {
                    ...
                    FileUtility.trasferisci_a_byte_file_binario(inSock, new DataOutputStream(outFile),true,numeri_da_utilizzare[cont]);
                    //System.out.println("File ricevuto correttamente in " + Duration.between(initialTime, LocalTime.now()).getSeconds() + " secondi");
                    long elapsed = (Duration.between(initialTime, LocalTime.now()).toMillis());
                    out.append(numeri_da_utilizzare[cont]+ "," + elapsed + "\n");
                    out.close();

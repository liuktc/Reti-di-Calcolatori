...
int main(int argc, char *argv[]) {
    int                sd, nread, port, num;
    char               c,nome_dir[LENGTH_FILE_NAME], nome_file[LENGTH_FILE_NAME];
    struct hostent    *host;
    ...
    while (gets(nome_dir)) {
        ...
        if (write(sd, nome_dir, (strlen(nome_dir))) < 0) {
            perror("write");
            break;
        }
        printf("Richiesta del file %s inviata... \n", nome_dir);

        // Continuo a leggere e a scrivere fino a quando non ricevo il fine linea
